            ...
            <el-form ref="formRef" :model="question_list" label-width=auto class="demo-dynamic">
                ...
                <el-space wrap size="large" direction="vertical" alignment="left">
                    <el-form-item v-for="(questions, index) in question_list.questions" :key="questions.key"
                                  :label="'questions' + index" :prop="'questions.' + index + '.value'" style="width: 60vw">
                        <el-card style="width: 100%">
                            ...
                            <el-card v-if="questions.type == '对象'" style="width: 100%">
                                ...
                                <el-form>
                                    <el-space wrap size="default" direction="vertical" alignment="left">
                                        <el-form-item v-for="(sub_questions, index) in questions.subproblem" :key="sub_questions.key"
                                                      :label="'sub_questions' + index" :prop="'sub_questions.' + index + '.value'" style="width: 50vw">
                                            <el-card style="width: 100%">
                                                ...
                                                <el-space wrap size="small" direction="vertical" alignment="left"
                                                    ...
                                                    <el-space wrap size="large" direction="horizontal">
                                                        ...
                                                        <el-button v-if="sub_questions.type == '数组'" @click="new_question(sub_questions)"
                                                                   type="success" round>新增一条
                                                        </el-button>
                                                        <el-button v-if="sub_questions.type == '数组'" @click="remove_question(sub_questions)"
                                                                   type="danger" round>删除最后一条
                                                        </el-button>
                                                    </el-space>
                                                    <el-form-item v-for="(sub_sub_questions, index) in sub_questions.subproblem"
                                                                  :key="sub_sub_questions.key" :label="'sub_sub_questions' + index"
                                                                  :prop="'sub_sub_questions.' + index + '.value'" style="width: 41vw">
                                                        <el-card style="width: 100%">
                                                            <p>{{ sub_sub_questions.stem }}</p>
                                                            <!--填空-->
                                                            <el-input v-if="sub_sub_questions.type == '填空'" v-model="sub_sub_questions.value"
                                                                      autosize />
                                                            <!--选择-->
                                                            <el-select v-if="sub_sub_questions.type == '选择'" v-model="sub_sub_questions.value"
                                                                       size="large">
                                                                <el-option v-for="item in sub_sub_questions.option" :key="item" :label="item"
                                                                           :value="item" />
                                                            </el-select>
                                                        </el-card>
                                                    </el-form-item>
                                                ...
                                                <el-select v-if="sub_questions.type == '选择'" v-model="sub_questions.value" size="large">
                                                    <el-option v-for="item in sub_questions.option" :key="item" :label="item" :value="item" />
                                                </el-select>
                                            </el-card>
                            ...
                            <el-space wrap size="default" direction="vertical" alignment="left">
                                <el-space wrap size="large" direction="horizontal">
                                    ...
                                    <el-button v-if="questions.type == '数组'" @click="new_question(questions)" type="success" round>新增一条
                                    </el-button>
                                    <el-button v-if="questions.type == '数组'" @click="remove_question(questions)" type="danger" round>删除最后一条
                                    </el-button>
                                </el-space>
                                <!--_______________数组的一道题_______________________-->

                                <el-form>
                                    <el-space wrap size="small" direction="vertical" alignment="left" v-if="questions.type == '数组'">
                                    ...
                                    <span v-if="sub_sub_questions.type == '数组'">{{
                                            sub_sub_questions.stem
                                                                        ...
                                                                        <el-button v-if="sub_sub_questions.type == '数组'"
                                                                                   @click="new_question(sub_sub_questions)" type="success" round>新增一条
                                                                        </el-button>
                                                                        <el-button v-if="sub_sub_questions.type == '数组'"
                                                                                   @click="remove_question(sub_sub_questions)" type="danger" round>删除最后一条
                                                                        </el-button>
                                                                    </el-space>
                                                                    <el-form-item v-for="(sub_sub_sub_questions, index) in sub_sub_questions.subproblem"
                                                                                  :key="sub_sub_sub_questions.key" :label="'sub_sub_sub_questions' + index"
                                                                                  :prop="'sub_sub_sub_questions.' + index + '.value'" style="width: 30vw">
                                                                        <el-card style="width: 100%">
                                                                            <p>{{ sub_sub_sub_questions.stem }}</p>
                                                                            <!--数组的 子对象的 子数组的 填空-->
                                                                            <el-input v-if="sub_sub_sub_questions.type == '填空'"
                                                                                      v-model="sub_sub_sub_questions.value" autosize />
                                                                            <!--数组的 子对象的 子数组的 选择-->
                                                                            <el-select v-if="sub_sub_sub_questions.type == '选择'"
                                                                                       v-model="sub_sub_sub_questions.value" size="large">
                                                                                <el-option v-for="item in sub_sub_sub_questions.option" :key="item" :label="item"
                                                                                           :value="item" />
                                                                            </el-select>
                                                                        </el-card>
                                                                    </el-form-item>
